...
模型V2版本 - 统一优化
主要优化点：
1. 缓存attn_mask（避免重复计算）
2. 优化MLP实现（减少reshape操作）
3. 浅层深层融合机制
4. 优化注意力机制
5. 使用更高效的激活函数
"""
...
# ==============================================================================
#  5. RSTB: Residual Swin Transformer Block（V2优化版）
# ==============================================================================
...
class RestorationSwinNetV2(nn.Module):
    """
    V2优化版图像恢复网络（集成DMSA_Fusion）
    1. 深度可分离卷积
    2. Mask缓存机制
    3. 优化的MLP实现
    4. �7�3 集成DMSA_Fusion双注意力多尺度融合模块
    5. 更高效的激活函数
    """
    def __init__(self, in_channels=3, dim=64, num_rstb=4, blocks_per_rstb=6, 
        ...
        # 集成：浅层特征提取（提高高度）
        self.shallow_conv = nn.Conv2d(dim, dim, 3, 1, 1)
        ...
        # 优化：conv_after_body使用深度可分离卷积
        if use_dwconv:
            self.conv_after_body = DepthwiseSeparableConv(dim, dim, kernel_size=3, padding=1)
        else:
            self.conv_after_body = nn.Conv2d(dim, dim, 3, 1, 1)
        
        # 集成：使用DMSA_Fusion进行浅层+深层特征融合（比简单的concat+conv）
        self.feature_fusion = DMSA_Fusion(dim)
            
        # 优化：tail使用深度可分离卷积
        if use_dwconv:
            self.tail = DepthwiseSeparableConv(dim, in_channels, kernel_size=3, padding=1)
        else:
...
class YCbCr_RestorationSwinNetV2(nn.Module):
    """
    YCbCr版本的V2恢复网络
    只处理Y通道，Cb/Cr通道直接保留（保持色彩不变）
    ...
    def __init__(self, dim=64, num_rstb=4, blocks_per_rstb=6, 
        ...
        super().__init__()
        # 核心网络只处理1通道（Y通道）
        self.core = RestorationSwinNetV2(
            in_channels=1,  # 改为1通道
            dim=dim,
            num_rstb=num_rstb,
    ...
    def forward(self, x_rgb, return_fusion_vis=False):
        ...
        Args:
            ...
            return_fusion_vis: 为 True 时由 core 返回融合过程特征（基于 Y 通道）
